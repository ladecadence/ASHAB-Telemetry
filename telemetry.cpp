...
QString Telemetry::toJSON()
{
    ...
    if (latitude.contains("N")) {
        latitude = latitude.remove(QChar('N'), Qt::CaseSensitive);
    } else {
        // S
        latitude = latitude.remove(QChar('S'), Qt::CaseSensitive).prepend("-");
    }
    if (longitude.contains("E")) {
        longitude = longitude.remove(QChar('E'), Qt::CaseSensitive);
    } else {
        // W
        longitude = longitude.remove(QChar('W'), Qt::CaseSensitive).prepend("-");
    }
